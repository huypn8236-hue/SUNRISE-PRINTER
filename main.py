...
def find_system_font():
    ...
    if is_android():
        # Trên Android, thử các font thông dụng
        font_paths = [
...
# ---------- TẠO ẢNH NHÃN (NGANG, 120x75mm) - BỐ CỤC CHUẨN ----------
def create_label_image(order_id, customer, box_index, box_total,
                       width_mm=120, height_mm=75, dpi=203):
    """
    Tạo ảnh nhãn ngang (120x75mm) với bố cục chuẩn, không bị tràn
    """
    if not HAS_PIL:
    ...
    # Nếu không tìm thấy font, dùng font mặc định
    if font_path:
        try:
            # Font với kích thước hợp lý, không tràn
            font_order = ImageFont.truetype(font_path, size=int(height_px * 0.25))    # Mã đơn: 50%
            font_name = ImageFont.truetype(font_path, size=int(height_px * 0.18))     # Tên khách: 42%
            font_box = ImageFont.truetype(font_path, size=int(height_px * 0.16))      # Box: 45%
        except:
            font_order = ImageFont.load_default()
            font_name = ImageFont.load_default()
            font_box = ImageFont.load_default()
    else:
        # Font mặc định của Pillow
        font_order = ImageFont.load_default()
        font_name = ImageFont.load_default()
        font_box = ImageFont.load_default()
    ...
    # Chia chiều cao thành 3 phần bằng nhau
